...
opts.add_option("-s", default='4',
                help="column in which strand information is located (1 being first), default=4")
...
if not options.o:
    ...
    if options.v == True:
        options.o = n1+'.'+n2+'.vect'
    elif options.i == True:
        options.o = n1+'.'+n2+'.iSize'
    else:
        options.o = n1+'.'+n2+'.vplot'
if options.u == True:
...
if options.v == True:
    ...
    plt.plot(np.convolve(mat, np.ones(int(options.window)), 'same') /
             int(options.window)/np.mean(mat[1:200]), 'r')
    ...
    plt.imshow(mat[0:yran, (int(options.e)-xran):(int(options.e)+xran+1)],
               origin='lower', aspect='equal', extent=[-xran, xran, 1, yran+1])
    plt.xlabel('Position relative to center')
